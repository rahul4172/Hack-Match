...
export default function ProfileEditPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto p-8">
      <h1 className="text-4xl font-extrabold mb-8 text-transparent bg-clip-text bg-gradient-to-r from-[#58A6FF] to-[#BC8CFF] font-mono">Profile_Config</h1>
      
      {/* Gamification / Status Section */}
      <div className="mb-10 ultra-glass p-6 rounded-2xl flex flex-col md:flex-row gap-8 items-center relative overflow-hidden">
        ...
        <div className="flex-shrink-0 text-center relative">
           <div className="w-24 h-24 rounded-full border-4 border-[#3FB950] border-dashed flex items-center justify-center bg-[#0D1117] text-4xl mb-2 animate-[spin_10s_linear_infinite]">
        ...
        <div className="flex-1 w-full">
          <div className="flex justify-between items-end mb-2 font-mono">
            <h2 className="text-2xl font-bold text-white text-glow">{formData.name || 'Anonymous User'}</h2>
          ...
          <div className="w-full bg-[#010409] rounded-full h-2.5 mb-6 border border-white/5 relative overflow-hidden">
      ...
      {msg && <div className="mb-4 p-3 bg-gray-800 border border-gray-700 text-cyan-300 rounded-md">{msg}</div>}
      
      <form onSubmit={handleSave} className="space-y-6 ultra-glass p-8 rounded-2xl relative z-10">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <label className="block text-sm text-gray-400 mb-1">Name</label>
            <input name="name" value={formData.name} onChange={handleChange} className="w-full bg-black/50 border border-white/10 rounded-lg p-3 text-white focus:border-cyan-500" />
          </div>
          <div>
            <label className="block text-sm text-gray-400 mb-1">Role / Title</label>
            <input name="role" value={formData.role} onChange={handleChange} className="w-full bg-black/50 border border-white/10 rounded-lg p-3 text-white focus:border-cyan-500" placeholder="e.g. Frontend Engineer" />
          </div>
        </div>

        <div>
          <label className="block text-sm text-gray-400 mb-1">Bio</label>
          <textarea name="bio" value={formData.bio} onChange={handleChange} className="w-full bg-black/50 border border-white/10 rounded-lg p-3 text-white focus:border-cyan-500 h-24" />
        </div>

        <div>
          <label className="block text-sm text-gray-400 mb-1">Skills (Comma separated)</label>
          <input name="skills" value={formData.skills} onChange={handleChange} className="w-full bg-black/50 border border-white/10 rounded-lg p-3 text-white focus:border-cyan-500" placeholder="React, Node, Python..." />
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <label className="block text-sm text-gray-400 mb-1">Recent Winnings / Achievements</label>
            <textarea name="winnings" value={formData.winnings} onChange={handleChange} className="w-full bg-black/50 border border-white/10 rounded-lg p-3 text-white focus:border-cyan-500 h-20" placeholder="1st Place at ETHDenver..." />
          </div>
          <div>
            <label className="block text-sm text-gray-400 mb-1">Current Learnings / Looking for</label>
            <textarea name="learnings" value={formData.learnings} onChange={handleChange} className="w-full bg-black/50 border border-white/10 rounded-lg p-3 text-white focus:border-cyan-500 h-20" placeholder="Learning Rust, looking for a Web3 team..." />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <input name="github" value={formData.github} onChange={handleChange} className="w-full bg-black/50 border border-white/10 rounded-lg p-3 text-white focus:border-cyan-500" />
          </div>
          <div>
            <label className="block text-sm text-gray-400 mb-1">LinkedIn Username</label>
            <input name="linkedin" value={formData.linkedin} onChange={handleChange} className="w-full bg-black/50 border border-white/10 rounded-lg p-3 text-white focus:border-cyan-500" />
          </div>
        </div>
